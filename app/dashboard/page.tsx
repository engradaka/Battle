...
import { validateInput, handleError } from "@/lib/error-handler"
...
function DashboardPageContent() {
  ...
  const checkUser = async () => {
    try {
      if (!sessionManager.checkPageAccess(window.location.pathname)) {
        return
      }
  ...
  const fetchCategoriesWithStats = async () => {
    ...
    const totalQuestions = processedCategories.reduce((sum: number, cat: any) => sum + (cat.question_count || 0), 0)
    const totalPoints = processedCategories.reduce((sum: number, cat: any) => sum + (cat.total_points || 0), 0)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      validateInput(formData.name_ar, 'Arabic name')
      validateInput(formData.name_en, 'English name')
    } catch (error) {
  ...
  const handleQuestionMediaChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      let validation
      if (questionFormData.question_type === 'image') {
        validation = FileValidator.validateImage(file)
      } else if (questionFormData.question_type === 'video') {
        validation = FileValidator.validateVideo(file)
      } else if (questionFormData.question_type === 'audio') {
        validation = FileValidator.validateAudio(file)
      }
  ...
  const handleQuestionSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedQuestionMedia && questionFormData.question_type !== 'text') {
      ...
      if (uploadError) {
        toast.error(`Error uploading media: ${uploadError.message}`)
        return
      }
    ...
    if (selectedAnswerMedia && questionFormData.answer_type !== 'text') {
      ...
      if (uploadError) {
        toast.error(`Error uploading answer media: ${uploadError.message}`)
        return
      }
  ...
  const handleSearch = async (query: string) => {
    ...
    if (!searchRateLimiter.isAllowed(user?.email || 'anonymous')) {
      toast.warning('Too many searches. Please wait a moment.')
      return
    }

    const sanitizedQuery = sanitizeForDB(query)
    const { data: questionsData, error } = await supabase
      ...
      `)
      .or(`question_ar.ilike.%${sanitizedQuery}%,question_en.ilike.%${sanitizedQuery}%,answer_ar.ilike.%${sanitizedQuery}%,answer_en.ilike.%${sanitizedQuery}%`)
      .limit(10)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col items-center sm:flex-row sm:items-center gap-3 mb-3 mt-4 sm:mt-0">
            ...
            <div className="min-w-0 flex-1 text-center sm:text-left">
              <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate">Quiz Admin Dashboard</h1>
              <p className="text-xs sm:text-sm text-gray-600 truncate">Manage categories & questions</p>
            </div>
          </div>

          <div className="flex flex-wrap gap-2">
            {user?.email === process.env.NEXT_PUBLIC_MASTER_ADMIN_EMAIL && (
              <Button
                onClick={() => router.push('/master-dashboard')}
                variant="outline"
                size="sm"
                className="flex-1 min-w-[120px] text-xs text-purple-600 border-purple-200"
              >
                ...
                {searchResults.map((question: any) => (
                  <div key={question.id} className="p-3 border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <p className="text-sm font-medium text-gray-900 mb-1">
                      Q: {sanitizeHTML(language === 'ar' ? question.question_ar : question.question_en)}
                    </p>
                    <p className="text-xs text-green-600">
                      A: {sanitizeHTML(language === 'ar' ? question.answer_ar : question.answer_en)}
                    </p>
                  </div>
                ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setQuestionFormData({ ...questionFormData, question_type: e.target.value as 'text' | 'video' | 'image' | 'audio' })}
                      className="w-full p-2 border border-gray-300 rounded-md"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setQuestionFormData({ ...questionFormData, answer_type: e.target.value as 'text' | 'video' | 'image' | 'audio' })}
                      className="w-full p-2 border border-gray-300 rounded-md"
                    >
                      <option value="text">Text Only</option>
                      <option value="image">Image</option>
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to delete this category? This will also delete all associated questions. This action cannot be undone."
        confirmText="Delete"
        cancelText="Cancel"
      />
    </div>
