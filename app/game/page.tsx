...
function getQuestionForCategoryPoint(
  ...
  // Iterate through usedQuestionIds (already ordered oldest first)
  for (const usedId of usedQuestionIds) {
    const question = questions.find(q => q.id === usedId)
    if (question) {
      return question // Returns the oldest used question for this category/diamond combo
    }
  }
...
export default function GamePage() {
  ...
  const [showPowerUpAnimation, setShowPowerUpAnimation] = useState<{
    powerUp: PowerUp
    team: 1 | 2
  } | null>(null)
  ...
  // Load used questions from Supabase (ordered oldest first)
  ...
  const availablePowerUps = {
    ...
    questionSwap: createPowerUp(
      'swap',
      'Question Swap',
      'Replace with different same-point question',
      '🔄'
    ),
    stealTurn: createPowerUp('steal', 'Steal Turn', 'Answer opponent question', '🎯'),
    blockQuestion: createPowerUp(
      'block',
      'Block Question',
      'Click any question to eliminate it',
      '🚫'
    ),
  }
  ...
  const handleQuestionClick = (question: Question) => {
    ...
    if (powerUpMode === 'swap') {
      ...
      const sameDiamondQuestions = questions.filter(
        q =>
      ...
      if (sameDiamondQuestions.length > 0) {
        const randomQuestion =
          sameDiamondQuestions[Math.floor(Math.random() * sameDiamondQuestions.length)]
  ...
  const handleAnswerResult = (teamNumber: number) => {
    ...
    const finalTeam1Score =
      teamNumber === 1 ? gameState.team1Score + finalDiamonds : gameState.team1Score
    const finalTeam2Score =
      teamNumber === 2 ? gameState.team2Score + finalDiamonds : gameState.team2Score
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          'linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.9)), url(/JW.png)',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="block sm:hidden mb-4">
          ...
          <div className="text-center mb-3">
            <div
              className={`inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-700 rounded-lg shadow-lg transition-all duration-300 ${
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="text-center">
              <div
                className={`relative overflow-hidden p-2 rounded-xl transition-all duration-300 ${
                  nextQuestionTurn === 1
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    ...
                    <h2 className="text-xs font-bold text-white truncate">{gameState.team1Name}</h2>
                  </div>
            ...
            <div className="text-center">
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-center gap-1 mb-1">
                    <Users className="w-3 h-3 text-white" />
                    <h2 className="text-xs font-bold text-white truncate">{gameState.team2Name}</h2>
                  </div>
                  <div className="text-sm font-bold text-white flex items-center justify-center gap-1">
                    <span>💎</span>
        ...
        <div className="hidden sm:flex justify-between items-center mb-2 sm:mb-3">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2">
            <div
              className={`inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-700 rounded-lg shadow-2xl transition-all duration-300 ${
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <h2 className="text-lg sm:text-xl font-bold text-white">{gameState.team1Name}</h2>
                </div>
          ...
          <div className="text-center">
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-center gap-2 mb-2">
                  <Users className="w-5 h-5 text-white" />
                  <h2 className="text-lg sm:text-xl font-bold text-white">{gameState.team2Name}</h2>
                </div>
                <div className="text-xl sm:text-2xl font-bold text-white flex items-center justify-center gap-2">
                  <span>💎</span>
                  <span>{gameState.team2Score}</span>
          ...
          {categories.map(category => (
            <Card key={category.id} className="overflow-hidden relative">
              <CardHeader className="bg-blue-800 text-white text-center py-2 sm:py-4 relative z-10 min-h-[3rem] sm:min-h-[4rem] flex items-center justify-center">
                <CardTitle className="text-sm sm:text-lg lg:text-xl font-bold line-clamp-2 leading-tight">
              ...
              >
                ...
                {category.image_url && <div className="absolute inset-0 bg-white/80"></div>}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
            <div className="animate-bounce">
              <div className="bg-yellow-400 rounded-full p-8 shadow-2xl animate-pulse">
                <div className="text-8xl animate-spin">{showPowerUpAnimation.powerUp.icon}</div>
              </div>
            </div>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <div className="bg-white rounded-lg p-4 shadow-xl animate-pulse">
                ...
                <p className="text-lg text-center mt-2">
                  <strong>{gameState[`team${showPowerUpAnimation.team}Name`]}</strong> got{' '}
                  <strong>{showPowerUpAnimation.powerUp.name}</strong>!
                </p>
              </div>
            </div>
        ...
        <Dialog open={!!selectedQuestion} onOpenChange={() => setSelectedQuestion(null)}>
          <DialogContent
            className="max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto rounded-xl"
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    >
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span
                          className={`text-xl sm:text-2xl font-bold ${timeLeft <= 10 ? 'text-red-500' : 'text-blue-600'}`}
                        >
                          {timeLeft}
                        </span>
                      </div>
                    ...
                    <div className="bg-blue-50 border-2 border-blue-300 rounded-lg px-4 py-2 sm:px-6 sm:py-3">
                      <div className="text-base sm:text-lg font-bold text-blue-800 text-center">
                        {currentTurn === 'finished' ? (
                          ...
                          <>
                            <span dir="ltr">
                              {currentTurn === 1 ? gameState.team1Name : gameState.team2Name}
                            </span>{' '}
                            دور
                          </>
                        ) : currentTurn === 1 ? (
                          `${gameState.team1Name}'s Turn`
                        ) : (
                      ...
                      {selectedQuestion.question_type === 'video' && (
                        <div className="flex justify-center">
                          <video
                            src={selectedQuestion.media_url}
                            controls
                            className="rounded-lg max-w-full h-auto max-h-48 md:max-h-64 shadow-md"
                          >
                  ...
                  {!showAnswer ? (
                    <div className="space-y-2">
                      <Button
                        ...
                        className="w-full h-12 sm:h-14 text-base sm:text-lg font-semibold bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-lg"
                      >
                        Show Answer
                      </Button>
                      {currentTurn === 'finished' && (
