...
import { logSecurityEvent } from "@/lib/activity-logger"
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        await logSecurityEvent(email, 'login_failed', { error: error.message })
      } else {
        // Get user session from Supabase
        const { data: { session: authSession } } = await supabase.auth.getSession()
        
        if (authSession?.user) {
          // Set new session with proper user ID
          const role = email === process.env.NEXT_PUBLIC_MASTER_ADMIN_EMAIL ? 'master_admin' : 'admin'
          sessionManager.setSession(authSession.user.id, email, role)
